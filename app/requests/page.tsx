...
const sample: RequestItem[] = [{ id: "demo-1", requestNo: "REQ-00124", material: "Valf", quantity: 500, unit: "adet", status: "REQUESTED", createdAt: new Date().toISOString() }];
...
export default function RequestsPage() {
  const [items, setItems] = useState<RequestItem[]>(sample);
  useEffect(() => { const saved = JSON.parse(localStorage.getItem("stoks_material_requests") ?? "[]") as RequestItem[]; if (saved.length) setItems(saved); }, []);

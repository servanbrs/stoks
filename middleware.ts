...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    const response = path.startsWith("/api/") ? NextResponse.json({ error: "Oturum geçersiz" }, { status: 401 }) : NextResponse.redirect(new URL(`/login?returnTo=${encodeURIComponent(path)}`, request.url));
